...
return
  ...
  await redlife.roles.remove(ayar.genel.erkekrolü1)
  }
  if(redlife.roles.cache.has(ayar.genel.erkekrolü2)){
  await redlife.roles.remove(ayar.genel.erkekrolü2)
  }
 await redlife.roles.remove(ayar.genel.kayıtsızrolü)
 await redlife.roles.remove(ayar.genel.kayıtsızrolü2)
 await redlife.roles.add(ayar.genel.kızrolü1)
 await redlife.roles.add(ayar.genel.kızrolü2)
...
db.add('isimlerkız.'+redlife.id, 1)
db.push("isimler." + redlife.id,  `\`${redlife.displayName}\` (<@&${ayar.genel.kızrolü1}>)\n`)
db.delete(`reds.${message.author.id}.${message.guild.id}.member`)
const redlife2 = new MessageEmbed()
.setColor(`RED`)
.setFooter(ayar.sunucuismi)
.setDescription(`${redlife} adlı kullanıcı başarıyla kadın olarak kaydedildi.`)
...
exports.conf = {enabled: true,guildOnly: true,aliases: ["kadın" , "k", "kız"],permLevel: 0}
exports.help = {name: 'Kadın',description: "Sunucuya kaydolmaya ne dersin ?",usage: 'Erkek isim yaş'} 